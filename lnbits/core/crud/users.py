...
async def create_user_nwc_config(
    user_id: str, 
    pubkey: str, 
    secret: str, 
    relay: str,
    ...
    """Create NWC configuration for a user"""
    from ..models.users import UserNWCConfig
    
    config_id = uuid4().hex
    config = UserNWCConfig(
        id=config_id,
        user_id=user_id,
        pubkey=pubkey,
        secret=secret,
    ...
    await (conn or db).insert("user_nwc_configs", config)
    return {
        "id": config.id,
...
async def get_user_nwc_config(
    user_id: str,
    conn: Connection | None = None
) -> dict | None:
    """Get NWC configuration for a user"""
    result = await (conn or db).fetchone(
        "SELECT * FROM user_nwc_configs WHERE user_id = :user_id",
        {"user_id": user_id}
    )
    return result

async def get_user_nwc_config_by_pubkey(
    pubkey: str,
    conn: Connection | None = None
) -> dict | None:
    """Get NWC configuration by user pubkey"""
    result = await (conn or db).fetchone(
        "SELECT * FROM user_nwc_configs WHERE pubkey = :pubkey",
        {"pubkey": pubkey}
    )
    return result

async def update_user_nwc_config(
    user_id: str,
    pubkey: str = None,
    secret: str = None,
    relay: str = None,
    ...
    """Update NWC configuration for a user"""
    updates = []
    values = {"user_id": user_id}
    
    if pubkey is not None:
        updates.append("pubkey = :pubkey")
        values["pubkey"] = pubkey
    if secret is not None:
        updates.append("secret = :secret")
        values["secret"] = secret
    if relay is not None:
        updates.append("relay = :relay")
        values["relay"] = relay
    ...
    query = f"UPDATE user_nwc_configs SET {', '.join(updates)} WHERE user_id = :user_id"
    await (conn or db).execute(query, values)
    
    return await get_user_nwc_config(user_id, conn)

async def delete_user_nwc_config(
    user_id: str,
    conn: Connection | None = None
) -> None:
    """Delete NWC configuration for a user"""
    await (conn or db).execute(
        "DELETE FROM user_nwc_configs WHERE user_id = :user_id",
        {"user_id": user_id}
    )
