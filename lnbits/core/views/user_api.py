...
# NWC Configuration Management Endpoints
@users_router.post("/user/{user_id}/nwc", name="Create or update NWC configuration for user")
async def api_users_create_user_nwc_config(
    user_id: str,
    request: Request,
    current_user: User = Depends(check_admin),
):
    """Create or update NWC configuration for a user"""
    try:
        ...
        relay = data.get("relay", "ws://localhost:7447")
        ...
        # Create or update NWC config
        from lnbits.core.crud.users import create_user_nwc_config, update_user_nwc_config, get_user_nwc_config
        
        existing_config = await get_user_nwc_config(user_id)
        if existing_config:
            # Update existing config
            result = await update_user_nwc_config(
                user_id=user_id,
                pubkey=pubkey,
                secret=new_secret,
                relay=relay
            ...
            # Create new config
            result = await create_user_nwc_config(
                user_id=user_id,
                pubkey=pubkey,
                secret=new_secret,
                relay=relay
        ...
        # Return NWC URI (without secret for security)
        nwc_uri = f"nostr+walletconnect://{result['pubkey']}?relay={result['relay']}&secret={new_secret}"
        ...
        logger.error(f"Error creating user NWC config: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")

@users_router.get("/user/{user_id}/nwc", name="Get NWC configuration for user")
async def api_users_get_user_nwc_config(
    user_id: str,
    current_user: User = Depends(check_admin),
):
    """Get NWC configuration for a user"""
    try:
        # Check if user exists
        user = await get_user(user_id)
        if not user:
        ...
        # Get NWC config
        from lnbits.core.crud.users import get_user_nwc_config
        config = await get_user_nwc_config(user_id)
        
        if not config:
            raise HTTPException(status_code=404, detail="NWC config not found")
        
        # Return config (without secret for security)
        return {
            "id": config["id"],
        ...
        logger.error(f"Error getting user NWC config: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")

@users_router.delete("/user/{user_id}/nwc", name="Delete NWC configuration for user")
async def api_users_delete_user_nwc_config(
    user_id: str,
    current_user: User = Depends(check_admin),
):
    """Delete NWC configuration for a user"""
    try:
        # Check if user exists
        user = await get_user(user_id)
        if not user:
        ...
        # Delete NWC config
        from lnbits.core.crud.users import delete_user_nwc_config
        await delete_user_nwc_config(user_id)
        
        return SimpleStatus(success=True, message="NWC config deleted successfully")
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error deleting user NWC config: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")

@users_router.get("/nwc/{pubkey}", name="Get NWC configuration by user pubkey")
async def api_users_get_nwc_config_by_pubkey(
    pubkey: str,
):
    """Get NWC configuration by user pubkey (public endpoint for NWC protocol)"""
    try:
        # Get NWC config by pubkey
        from lnbits.core.crud.users import get_user_nwc_config_by_pubkey
        config = await get_user_nwc_config_by_pubkey(pubkey)
        
        if not config:
            raise HTTPException(status_code=404, detail="NWC config not found")
        
        # Return public config info
        return {
            "pubkey": config["pubkey"],
            "relay": config["relay"],
            "created_at": config["created_at"]
        ...
        logger.error(f"Error getting NWC config by pubkey: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
