...
class MultiUserNWCWallet(Wallet):
    """
    A multi-user NWC wallet that supports multiple users with independent NWC connections.
    Each user can have their own NWC pairing and wallet operations.
    """

    def __init__(self):
        self.shutdown = False
        self.user_connections = {}  # Store NWC connections for each user
        self.pending_payments = {}  # Track pending payments per user
        self.paid_invoices_queue = asyncio.Queue(0)
    ...
    async def _get_user_connection(self, user_pubkey: str) -> NWCConnection:
        """Get or create NWC connection for a specific user"""
        if user_pubkey not in self.user_connections:
            # Get user's NWC config from database
            from lnbits.core.crud.users import get_user_nwc_config_by_pubkey
            from lnbits.core.crud.users import create_user_nwc_config
            from lnbits.settings import settings
            
            user_nwc = await get_user_nwc_config_by_pubkey(user_pubkey)
            
            if not user_nwc:
                # Create new NWC config for user
                import secrets
                new_secret = secrets.token_hex(32)  # 64 characters
                ...
                if not accounts.items:
                    raise ValueError(f"User with pubkey {user_pubkey} not found")
            ...
            # Create NWC connection for this user
            self.user_connections[user_pubkey] = NWCConnection(
                user_nwc["pubkey"],
                user_nwc["secret"],
                user_nwc["relay"]
            )
            
            # Initialize pending payments for this user
            self.pending_payments[user_pubkey] = []
        
        return self.user_connections[user_pubkey]

    async def _handle_pending_payments(self):
        """Periodically check if any pending payments have been settled."""
        while not self._is_shutting_down():
            await asyncio.sleep(10)  # Check every 10 seconds
            
            for user_pubkey, payments in self.pending_payments.items():
                if not payments:
                    continue
                
                try:
                    conn = self.user_connections.get(user_pubkey)
                    if not conn:
                        continue
                    
                    now = time.time()
                    for payment in payments[:]:  # Copy list to avoid modification during iteration
                        try:
                            if not payment["settled"]:
                                payment_data = await conn.call(
                                    "lookup_invoice", {"payment_hash": payment["checking_id"]}
                                )
                                settled = (
                                    "settled_at" in payment_data
                                ...
                                if settled:
                                    logger.debug(
                                        f"Pending payment {payment['checking_id']} settled for user {user_pubkey}"
                                    )
                                    payment["settled"] = True
                                    self.paid_invoices_queue.put_nowait({
                        ...
                        try:
                            if now > payment["expires_at"]:
                                logger.warning(
                                    f"Pending payment {payment['checking_id']} timed out for user {user_pubkey}"
                                )
                                payment["expired"] = True
                        except Exception as e:
                            logger.error(f"Error handling pending payment timeout for user {user_pubkey}: {str(e)}")
                    
                    # Remove settled or expired payments
                    self.pending_payments[user_pubkey] = [
                        payment for payment in payments
                        if not payment.get("settled") and not payment.get("expired")
                    ]
    ...
    async def cleanup(self):
        ...
        for conn in self.user_connections.values():
            try:
                ...
                logger.warning(f"Error closing NWC connection: {str(e)}")
    ...
    async def create_invoice(
        ...
        **_
        ...
        try:
            conn = await self._get_user_connection(user_pubkey)
            info = await conn.get_info()
            
            if "make_invoice" not in info["supported_methods"]:
                return InvoiceResponse(
                    error_message="make_invoice method not supported by this NWC service"
                )
            ...
            return InvoiceResponse(
                ...
                payment_request=invoice_data["invoice"],
                error_message=None,
            )
            
        except Exception as e:
            msg = f"Error creating invoice for user {user_pubkey}: {str(e)}"
            logger.error(msg)
            return InvoiceResponse(error_message=msg)
    ...
    async def pay_invoice(
        ...
        **_
        ...
        """Pay invoice for a specific user"""
        try:
            conn = await self._get_user_connection(user_pubkey)
            info = await conn.get_info()
            
            if "pay_invoice" not in info["supported_methods"]:
                return PaymentResponse(
                    error_message="pay_invoice method not supported by this NWC service"
                )
            
            # Decode invoice to get payment hash
            decoded = bolt11_decode(payment_request)
            if not decoded:
                return PaymentResponse(error_message="Invalid invoice")
            ...
            if user_pubkey not in self.pending_payments:
                self.pending_payments[user_pubkey] = []
            
            self.pending_payments[user_pubkey].append({
                "checking_id": payment_hash,
                "amount": amount,
                "settled": False,
            ...
            return PaymentResponse(
                ok=True,
                checking_id=payment_hash,
                fee_msat=payment_data.get("fees_paid", 0),
                error_message=None,
            )
            
        except Exception as e:
            msg = f"Error paying invoice for user {user_pubkey}: {str(e)}"
            logger.error(msg)
            return PaymentResponse(error_message=msg)
    ...
    async def get_invoice_status(
        ...
        """Get invoice status for a specific user"""
        try:
            conn = await self._get_user_connection(user_pubkey)
            return await conn.get_payment_status(checking_id)
        except Exception as e:
            logger.error(f"Error getting invoice status for user {user_pubkey}: {str(e)}")
            return PaymentStatus(None, fee_msat=None, preimage=None)

    async def get_balance(self, user_pubkey: str) -> int:
        """Get balance for a specific user"""
        try:
            conn = await self._get_user_connection(user_pubkey)
            info = await conn.get_info()
            
            if "get_balance" in info["supported_methods"]:
                balance_data = await conn.call("get_balance", {})
                return balance_data.get("balance", 0)
            else:
                return 0
        except Exception as e:
            logger.error(f"Error getting balance for user {user_pubkey}: {str(e)}")
            return 0

    async def paid_invoices_stream(self) -> AsyncGenerator[dict, None]:
        """Stream of paid invoices for all users"""
        while not self._is_shutting_down():
            try:
                value = await self.paid_invoices_queue.get()
                yield value
            except Exception as e:
                logger.error(f"Error in paid invoices stream: {str(e)}")
                await asyncio.sleep(1)
